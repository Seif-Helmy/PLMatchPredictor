...
def clean_fixtures_table(csv_file):
    ...
    predictors = ["Venue", "OppCode", "ClubCode"]
    ...
    past_fixtures_with_rolling = past_fixtures.groupby("Club", group_keys=True).apply(
        lambda x: rolling_averages(x, predictors_to_be_rolled, new_predictors_rolling, 10), include_groups=False)
...
def rolling_averages(group, stats, rolling_cols, games=10):
    ...
    rolling_stats = group[stats].rolling(games, closed="left").mean()
...
clean_fixtures_table("AllFixtures.csv")
